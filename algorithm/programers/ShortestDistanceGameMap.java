...
public class ShortestDistanceGameMap {
    ...
    public int solution(int[][] maps) {
        ...
        while (!que.isEmpty()) {
            ...
            if (!visit[c.x][c.y]) {
                visit[c.x][c.y] = true;
                for (int i = 0; i < 4; i++) {
                    int moveX = c.x + addX[i], moveY = c.y + addY[i];
                    if (isValid(moveX, moveY, n, m, maps)) {
                        que.add(new Character(moveX, moveY, c.distance + 1));
                    }
                }
            }
        }
    ...
    public boolean isValid(int moveX, int moveY, int n, int m, int[][] maps) {
        return moveX >= 0 && moveY >= 0 && moveX < n && moveY < m && maps[moveX][moveY] == 1;
    }
    ...
    public class Character {
        ...
        Character(int x, int y, int distance) {
            this.x = x;
            this.y = y;
