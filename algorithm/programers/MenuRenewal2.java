...
public class MenuRenewal2 {
    ...
    Map<String, Integer> numberOfMenus;
    ...
    public String[] solution(String[] orders, int[] course) {
        ...
        for (int numberOfCourse : course) {
            numberOfMenus = new HashMap<>();
            for (String order : orders) {
                /* 주문된 코스 메뉴들 구하기 */
                makeCourse("", order, numberOfCourse);
            }

            /* 주문된 메뉴 모음 중 2회 주문 이상이면서 최대인 횟수 구하기 */
            Optional<Integer> optMaxValue = numberOfMenus.entrySet().stream()
                    .filter(e -> e.getValue() >= 2)
                    .max(Map.Entry.comparingByValue())
                    .map(Map.Entry::getValue);

            /* 최대 주문수의 메뉴 모음만 구해서 담기 */
            optMaxValue.ifPresent(
                    i -> result.addAll(
                            numberOfMenus.entrySet().stream()
                            .filter(e -> Objects.equals(e.getValue(), i))
    ...
    public void makeCourse(String menus, String order, int numberOfCourse) {
        if(menus.length() >= numberOfCourse) {
            numberOfMenus.put(menus, numberOfMenus.getOrDefault(menus, 0) + 1);
            return;
        }

        for (int i = 0; i < order.length(); i++) {
            makeCourse(menus + order.charAt(i), order.substring(i + 1), numberOfCourse);
        }
    }
