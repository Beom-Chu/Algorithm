...
"java and backend and junior and pizza 100" : java로 코딩테스트를 봤으며, backend 직군을 선택했고 junior 경력이면서
소울푸드로 pizza를 선택한 지원자 중 코딩테스트 점수를 100점 이상 받은 지원자는 1명 입니다.
...
"cpp and - and senior and pizza 250" : cpp로 코딩테스트를 봤으며, senior 경력이면서 소울푸드로 pizza를 선택한 지원자 중
...
public class RankSearch {
    ...
    public int[] solution(String[] info, String[] query) {
        ...
        List<Item> applicants = new ArrayList<>();
        List<Item> queries = new ArrayList<>();

        for(String s : info) { applicants.add(new Item(s)); }
        for(String s : query) { queries.add(new Item(s)); }

        for (int i = 0; i < queries.size(); i++) {
            Item q = queries.get(i);
            answer[i] = (int) applicants.stream()
                    .filter(item -> (q.language.equals("-") || item.language.equals(q.language))
                            && (q.jobGroup.equals("-") || item.jobGroup.equals(q.jobGroup))
                            && (q.career.equals("-") || item.career.equals(q.career))
                            && (q.soulFood.equals("-") || item.soulFood.equals(q.soulFood))
                            && item.score >= q.score)
                    .count();
        }
        return answer;
    ...
    public class Item {
        ...
        Item(String info) {
            StringTokenizer st = new StringTokenizer(info.replaceAll(" and "," "));
            this.language = st.nextToken();
            this.jobGroup = st.nextToken();
            this.career = st.nextToken();
            this.soulFood = st.nextToken();
