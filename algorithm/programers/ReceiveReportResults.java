...
public class ReceiveReportResults {
    ...
    public int[] solution(String[] id_list, String[] report, int k) {
        ...
        Map<String, List<String>> idAndReports = new HashMap<>();
        Map<String, Integer> reportCnt = new HashMap<>();

        for(String r : report) {
            StringTokenizer st = new StringTokenizer(r);
            String id = st.nextToken();
            String toReport = st.nextToken();

            List<String> reports = idAndReports.getOrDefault(id, new ArrayList<>());

            if(!reports.contains(toReport)) {
                reports.add(toReport);
                idAndReports.put(id, reports);
                reportCnt.put(toReport, reportCnt.getOrDefault(toReport, 0) + 1);
            }
        }

        System.out.println("[[[idAndReports = " + idAndReports);
        System.out.println("[[[reportCnt = " + reportCnt);

        for(Map.Entry<String, Integer> stopUser : reportCnt.entrySet()) {
            if(stopUser.getValue() >= k) {
                for (int i = 0; i < id_list.length; i++) {
                    if (idAndReports.get(id_list[i]) != null
                            && idAndReports.get(id_list[i]).contains(stopUser.getKey())) {
                        answer[i]++;
                    }
                }
            }
