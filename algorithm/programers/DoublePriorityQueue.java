...
public class DoublePriorityQueue {
  ...
  public int[] solution2(String[] operations) {
    ...
    List<Integer> dpq = new ArrayList<>();

    for (String oper : operations) {
      String[] operSplit = oper.split(" ");

      if (operSplit[0].equals("I")) {
        dpq.add(Integer.valueOf(operSplit[1]));

      } else {
        if (operSplit[1].compareTo("0") > 0) {
          dpq.sort((i1, i2) -> i2 - i1);
        } else {
          dpq.sort((i1, i2) -> i1 - i2);
        }
        dpq.remove(0);
      }
    }
