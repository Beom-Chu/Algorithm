...
public class ChangeWords {
  ...
  public int solution2(String begin, String target, String[] words) {
    ...
    HashMap<String, Integer> wordDepth = new HashMap<>();

    stk.add(begin);
    wordDepth.put(begin, 0);
    wordDepth.put(target, 0);
    ...
    while (!stk.isEmpty()) {
      ...
      for (int i = 0; i < words.length; i++) {
        ...
        if (!chk[i]) {
          if (changable(word, words[i])) {
            chk[i] = true;
            stk.add(words[i]);
            wordDepth.put(words[i], wordDepth.get(word) + 1);
          }
        }
      }
    }

    return wordDepth.get(target);
  }
...
//     String[] words = {"hot", "dot", "dog", "lot", "log", "cog"}; /*4*/
    ...
    System.out.println(cw.solution2(begin, target, words));
  }
