...
public class MaximizeFormulas {
    ...
    public long solution(String expression) {
        ...
        LinkedList<Long> numbers = Arrays.stream(expression.split("[+\\-*]"))
                .mapToLong(Long::parseLong)
                .boxed()
                .collect(Collectors.toCollection(LinkedList::new));

        LinkedList<String> operators = Arrays.stream(expression.split("[0-9]"))
                .filter(s->!s.equals(""))
                .collect(Collectors.toCollection(LinkedList::new));

        for(String[] priority : priorities) {
            LinkedList<Long> tempNumbers = new LinkedList<>(numbers);
            LinkedList<String> tempOperators = new LinkedList<>(operators);

            for(String p : priority) {
                calculation(p, tempNumbers, tempOperators);
            }
            result = Math.max(result, Math.abs(tempNumbers.get(0)));
        }

        return result;
    }

    public void calculation(String operator, LinkedList<Long> numbers, LinkedList<String> operators) {

        while(operators.contains(operator)) {
            int idx = operators.indexOf(operator);
            long calValue = calculation(operator, numbers.get(idx), numbers.get(idx + 1));
            operators.remove(idx);
            numbers.remove(idx);
            numbers.remove(idx);
            numbers.add(idx, calValue);
        }
    }

    public long calculation(String operator, long num1, long num2) {
        return operator.equals("+") ? (num1 + num2) : operator.equals("-") ? (num1 - num2) : (num1 * num2);
    }
    ...
    public void test() {
        ...
        Assertions.assertEquals(6, solution("1+2+3"));
    }
}
