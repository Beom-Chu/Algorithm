...
public class NewsClustering {
    ...
    public int solution(String str1, String str2) {
        ...
        Map<String, Integer> setStr1 = makeMultipleSet(str1.toLowerCase());
        Map<String, Integer> setStr2 = makeMultipleSet(str2.toLowerCase());

        System.out.println("[[[setStr1 = " + setStr1);
        System.out.println("[[[setStr2 = " + setStr2);

        Map<String, Integer> union = new HashMap<>(setStr1);
        Map<String, Integer> intersection = new HashMap<>();

        for(Map.Entry<String, Integer> e : setStr2.entrySet()) {
            union.put(e.getKey(), union.get(e.getKey()) == null ? e.getValue() : Math.max(union.get(e.getKey()), e.getValue()));
        }

        for(Map.Entry<String, Integer> e1 : setStr1.entrySet()) {
            for(Map.Entry<String, Integer> e2 : setStr2.entrySet()) {
                if(e1.getKey().equals(e2.getKey())) {
                    intersection.put(e1.getKey(), Math.min(e1.getValue(), e2.getValue()));
                }
            }
        }
        ...
        for(Map.Entry<String, Integer> e : union.entrySet()) {
            unionCnt += e.getValue();
        }

        for(Map.Entry<String, Integer> e : intersection.entrySet()) {
            intersectionCnt += e.getValue();
        }

        System.out.println("[[[unionCnt = " + unionCnt);
        System.out.println("[[[intersectionCnt = " + intersectionCnt);

        return unionCnt == 0 ? 65536 : 65536 * intersectionCnt / unionCnt;
    }

    public Map<String, Integer> makeMultipleSet(String str) {
        Map<String, Integer> set = new HashMap<>();

        for (int i = 0; i < str.length() - 1; i++) {
            String substr = str.substring(i, i+2);
            if(substr.matches("^[a-z]*$")) {
                set.put(substr, set.getOrDefault(substr, 0) + 1);
            }
        }
