...
public class DFSAndBFS {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < m; i++) {
            int x = readInt();
            int y = readInt();
            connect.add(new Connect(x, y));
            connect.add(new Connect(y, x));
        }

        connect.sort((o1, o2) -> o1.from == o2.from ? o1.to - o2.to : o1.from - o2.from);
    ...
    private static void dfs(List<Connect> connect, int v) {
        visit[v] = true;
        dfs.add(String.valueOf(v));

        for (Connect c : connect) {
            if(c.from == v && !visit[c.to]) {
                dfs(connect, c.to);
            }
        }
    }

    private static void bfs(List<Connect> connect, int v) {
