...
1 초	128 MB	97305	53933	45561	55.931%
문제
...
public class BodySize {
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            bodies[i] = new Body(reader.readLine());
        }
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                if(i != j && bodies[j].weight > bodies[i].weight && bodies[j].height > bodies[i].height) {
                    bodies[i].rank++;
                }
            }
            joiner.add(String.valueOf(bodies[i].rank));
        }
