...
1 초	256 MB	    23818	8245	5985	35.557%
문제
...
public class ConveyorBeltSushi {
    public static void main(String[] args) throws IOException {
        int n = readInt(); // 접시수
        int d = readInt(); // 초밥 가지수
        int k = readInt(); // 연속해서 먹는 접시의 수
        int c = readInt(); // 쿠폰번호
        int[] sushi = new int[n + k - 1];
        List<Integer> eat = new LinkedList<>();
        eat.add(c);
        ...
        int result = getCount(eat);

        for (int i = k; i < sushi.length; i++) {
            eat.remove(1);
            eat.add(sushi[i]);
            result = Math.max(result, getCount(eat));
        }

        System.out.println(result);
    }

    public static int getCount(List<Integer> eat) {
        return new HashSet<>(eat).size();
    }
