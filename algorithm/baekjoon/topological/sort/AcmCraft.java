...
public class AcmCraft {
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < testCount; i++) {
      testCases.add(new TestCase(reader));
    }
  ...
  /* 위상정렬 */
  public static void topologicalSort(TestCase t) {
    Queue<Integer> que = new LinkedList<>();
    int[] totalTime = new int[t.buildingCount + 1];

    for (int i = 1; i <= t.buildingCount; i++) {
      if(t.inDegree[i] == 0) {
        que.add(i);
        totalTime[i] = t.buildTime[i];
      }
    }

    while(!que.isEmpty()) {
      Integer from = que.poll();
      if(from == t.lastBuilding) {
        System.out.println(totalTime[from]);
        break;
      }

      for(Integer to : t.buildRules.get(from)) {
        t.inDegree[to]--;
        if(t.inDegree[to] == 0) {
          que.add(to);
        }
        totalTime[to] = Math.max(totalTime[to], totalTime[from] + t.buildTime[to]);
      }
    }
  }
  ...
  public static class TestCase {
    ...
    int[] buildTime;
    int[] inDegree;
    Map<Integer, List<Integer>> buildRules;
    int lastBuilding;

    public TestCase(BufferedReader reader) throws IOException {
      StringTokenizer st1 = new StringTokenizer(reader.readLine());
      this.buildingCount = Integer.parseInt(st1.nextToken());
      this.ruleCount = Integer.parseInt(st1.nextToken());

      StringTokenizer st2 = new StringTokenizer(reader.readLine());
      this.buildTime = new int[buildingCount+1];
      this.inDegree = new int[buildingCount+1];
      this.buildRules = new HashMap<>();

      for (int i = 1; i <= buildingCount; i++) {
        this.buildTime[i] = Integer.parseInt(st2.nextToken());
        buildRules.put(i, new ArrayList<>());
      }
      ...
      for (int i = 0; i < ruleCount; i++) {
        ...
        buildRules.get(from).add(to);
        inDegree[to]++;
      }

      this.lastBuilding = Integer.parseInt(reader.readLine());
    }
  }
}
