...
2 초	128 MB	20365	14952	12763	74.865%
문제
...
public class Bar {
    ...
    public static void solution(int X) {
        ...
        while(pq.stream().mapToInt(i->i).sum() > X) {
            int min = pq.poll();
            int div = min / 2;

            pq.add(div);

            if(pq.stream().mapToInt(i->i).sum() < X) {
                pq.add(div);
            }
        }
    ...
    public static void solution3(int X) {
        int count = 0;
        for (int i = 0; i < 7; i++) {
            if((X & (1<<i)) > 0) {
                count++;
    ...
    public static void solution4(int X) {
        System.out.println(Integer.toBinaryString(X).replaceAll("0","").length());
    }

    public static void solution5(int X) {
