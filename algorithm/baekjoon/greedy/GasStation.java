...
2 초	512 MB	65151	25421	19815	38.512%
문제
...
public class GasStation {
    // 왼쪽 도시부터 오른쪽으로 이동하면서 주유소에 들를 때마다 가장 저렴한 주유소의 금액으로 이동거리를 곱해준다.
    public static void main(String[] args) throws IOException {
        BufferedReader reader = new BufferedReader(new InputStreamReader(System.in));
        int numberOfCities = Integer.parseInt(reader.readLine());
        long[] distances = Arrays.stream(reader.readLine().split(" ")).mapToLong(Long::parseLong).toArray();
        long[] gasPrices = Arrays.stream(reader.readLine().split(" ")).mapToLong(Long::parseLong).toArray();

        long cheapestGasPrice = Long.MAX_VALUE;
        long total = 0;

        for (int i = 0; i < numberOfCities - 1; i++) {
            cheapestGasPrice = Math.min(cheapestGasPrice, gasPrices[i]);
            total += distances[i] * cheapestGasPrice;
        }
