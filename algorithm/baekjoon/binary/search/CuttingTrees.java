...
1 초	256 MB	118593	33783	21004	25.545%
문제
...
public class CuttingTrees {
    public static void main(String[] args) throws IOException {
        ...
        int requiredTreeLength = Integer.parseInt(st1.nextToken());
        int[] heightOfTrees = new int[treeCount];
        ...
        System.out.println(binarySearch(heightOfTrees, requiredTreeLength));
    }

    public static int binarySearch(int[] heightOfTrees, int requiredTreeLength) {

        int l = 0, h = heightOfTrees[heightOfTrees.length - 1];

        while (l < h) {
            int m = (l + h) / 2;
            long cuttingTreeLength = cuttingTrees(heightOfTrees, m);
            if (cuttingTreeLength < requiredTreeLength) {
                h = m;
            } else {
                l = m + 1;
            }
    ...
    public static long cuttingTrees(int[] heightOfTrees, int cutterHeight) {
        return Arrays.stream(heightOfTrees).mapToLong(i->i > cutterHeight ? i - cutterHeight : 0).sum();
    }
}
/*
3 1
