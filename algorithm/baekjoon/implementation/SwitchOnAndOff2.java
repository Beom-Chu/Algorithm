...
public class SwitchOnAndOff2 {
    public static void main(String[] args) throws IOException {
        BufferedReader reader = new BufferedReader(new InputStreamReader(System.in));
        int switchCount = Integer.parseInt(reader.readLine());
        boolean[] switches = new boolean[switchCount];
        StringTokenizer stSwitches = new StringTokenizer(reader.readLine());
        for (int i = 0; i < switchCount; i++) {
            switches[i] = stSwitches.nextToken().equals("1");
        }
        ...
        for (Student student : students) {
            turnOnSwitch(student, switches);
        }

        print(switches);
    }

    private static void print(boolean[] switches) {
        StringJoiner sj = new StringJoiner(" ");
        for (int i = 0; i < switches.length; i++) {
            sj.add(switches[i] ? "1" : "0");
            if((i + 1) % 20 == 0) {
    ...
    private static void turnOnSwitch(Student student, boolean[] switches) {
        if(student.gender == 1) {
            turnOnSwitchMale(student.number, switches);
        } else {
            turnOnSwitchFemale(student.number, switches);
        }
    }

    private static void turnOnSwitchMale(int number, boolean[] switches) {
        for (int i = 0; i < switches.length; i++) {
            if((i + 1) % number == 0) {
    ...
    private static void turnOnSwitchFemale(int number, boolean[] switches) {
        int idx = number - 1;
        int left = idx, right = idx;
        switches[idx] = !switches[idx];

        while (0 < left && right < switches.length - 1) {
            left--;
            right++;
...
/*
22
0 1 0 1 0 0 0 1 0 1 0 1 0 0 0 1 1 1 0 1 0 1
2
1 3
2 3
 */