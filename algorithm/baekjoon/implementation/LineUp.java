...
1 초	256 MB	4333	2301	1988	54.721%
문제
...
public class LineUp {
    ...
    public static void main(String[] args) throws IOException {
        ...
        List<StringTokenizer> inputs = new ArrayList<>();
        for (int i = 0; i < P; i++) {
            inputs.add(new StringTokenizer(reader.readLine()));
        }

        for (StringTokenizer st : inputs) {
            lineUp(st);
        }
    }

    private static void lineUp(StringTokenizer st) {
        ...
        while (st.hasMoreTokens()) {
            ...
            for (int i = 0; i < lineUp.size(); i++) {
                if (lineUp.get(i) > height) {
                    moveCount += lineUp.size() - i;
                    lineUp.add(i, height);
                    break;
                }
            }
            if (!lineUp.contains(height)) {
                lineUp.add(height);
            }
        }
        System.out.println(lineNumber + " " + moveCount);
    }
}
